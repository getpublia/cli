...
const CONFIG_DIR = path.join(os.homedir(), ".config", "publia");
const CONFIG_FILE = path.join(CONFIG_DIR, "config.json");
const DEFAULT_SERVER_URL = "https://getpublia.com";
...
function cliEnv(name) {
  return process.env[`PUBLIA_${name}`] ?? process.env[`KNBOX_${name}`];
}
...
Usage:
  publia auth login [--server <url>]     Sign in through browser OAuth
  publia auth logout                     Remove the local CLI token
  publia auth token                      Print the token for scripts
  publia auth whoami [--json]            Show the current user
  publia commands --json                 Print the command catalog
  publia login [--server <url>]          Sign in through browser OAuth
  publia logout                          Remove the local CLI token
  publia whoami [--json]                 Show the current user
  publia ls [path] [--json]              List files and directories
  publia cd <path>                       Change the saved remote directory
  publia open [path] [--browser] [--json]
                                      Print a file URL or list a directory
  publia upload <file-or-dir> [options]  Upload a file or directory
...
Global options:
  --server <url>    Publia server URL (or $PUBLIA_URL)
...
Environment:
  PUBLIA_URL         Default server URL
  PUBLIA_TOKEN       Bearer token for non-interactive agent calls
`;
...
async function auth(args) {
  ...
  if (subcommand === "help") {
    console.log("Usage: publia auth <login|logout|whoami|token>");
    return;
  }
...
async function login(args) {
  ...
  const serverUrl = cleanServerUrl(args.server || cliEnv("URL") || existing.serverUrl || DEFAULT_SERVER_URL);
  ...
  console.error(`Opening browser for Publia login: ${loginUrl}`);
  ...
  if (args.json) {
    printJsonOutput(args, { serverUrl, username: next.username }, "Logged in", [
      { action: "whoami", cmd: "publia auth whoami --json" },
      { action: "list", cmd: "publia ls --json" },
    ]);
  } else {
...
async function logout(args) {
  ...
  if (args.json) printJsonOutput(args, { revoked: true }, "Logged out", [{ action: "login", cmd: "publia auth login" }]);
...
async function whoami(args) {
  ...
  if (args.json) {
    printJsonOutput(args, { serverUrl: config.serverUrl, cwd: config.cwd, user: result.user }, result.user.username, [
      { action: "list", cmd: `publia ls ${shellPath(config.cwd)} --json` },
      { action: "upload", cmd: "publia upload <path> --json" },
    ]);
  }
  else {
...
async function token(args) {
  const config = await getRuntimeConfig(args);
  if (args.json) {
    printJsonOutput(args, { token: config.token, source: cliEnv("TOKEN") ? "env" : "config" }, "Token loaded");
  } else {
...
async function changeDirectory(args) {
  ...
  if (args.json) {
    printJsonOutput(args, { cwd: target }, `Changed directory to /${target}`, [
      { action: "list", cmd: `publia ls ${shellPath(target)} --json` },
      { action: "upload", cmd: `publia upload <path> --to ${shellPath(target)} --json` },
    ]);
  }
...
async function openRemote(args) {
  ...
  if (args.json) {
    printJsonOutput(args, { kind: item.kind, path: item.path, url: item.url }, item.url, [
      { action: "open_browser", cmd: `publia open ${shellPath(item.path)} --browser` },
      { action: "list_parent", cmd: `publia ls ${shellPath(parentRemotePath(item.path))} --json` },
    ]);
  }
...
async function upload(args) {
  ...
  if (!local) throw new CliError("Usage: publia upload <file-or-dir>");
...
async function getRuntimeConfig(args, { requireToken = true } = {}) {
  ...
  const serverUrl = cleanServerUrl(args.server || cliEnv("URL") || config.serverUrl || DEFAULT_SERVER_URL);
  const token = cliEnv("TOKEN") || config.token;
  if (requireToken && !token) throw new CliError("Not logged in. Run `publia login` or set PUBLIA_TOKEN.");
...
async function createLoginListener({ state, serverUrl }) {
  ...
  const done = new Promise((resolve, reject) => {
    ...
    server = http.createServer((req, res) => {
      ...
      res.end("<!doctype html><title>Publia CLI</title><p>Publia CLI login complete. You can close this window.</p>");
      clearTimeout(timeout);
      server.close();
      resolve({
        token,
...
function listingBreadcrumbs(listing) {
  ...
  if (listing.parent !== null && listing.parent !== undefined) {
    crumbs.push({ action: "up", cmd: `publia cd ${shellPath(listing.parent)} && publia ls --json` });
  }
  for (const item of listing.items.slice(0, 10)) {
    crumbs.push({
      action: item.kind === "directory" ? "enter" : "open",
      cmd: item.kind === "directory" ? `publia cd ${shellPath(item.path)}` : `publia open ${shellPath(item.path)} --json`,
    });
  }
  return crumbs;
}
...
function uploadBreadcrumbs(uploaded, entryUrl) {
  const crumbs = [];
  if (entryUrl) crumbs.push({ action: "open_site", cmd: `publia open ${shellPath(parentRemotePath(uploaded.find((file) => /(^|\/)index\.html?$/i.test(file.path))?.path || ""))} --json` });
  for (const file of uploaded.slice(0, 10)) {
    crumbs.push({ action: "open", cmd: `publia open ${shellPath(file.path)} --json` });
  }
  return crumbs;
}
...
function agentHelp() {
  return {
    ...
    data: {
      name: "publia",
      purpose: "Upload and browse files in Publia from terminals and AI agents.",
      config: {
        env: ["PUBLIA_URL", "PUBLIA_TOKEN"],
    ...
    summary: "Publia CLI agent help",
    breadcrumbs: [{ action: "catalog", cmd: "publia commands --json" }],
  };
}
